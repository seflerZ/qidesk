# Android.mk for libvterm + JNI bridge.
#
# Two libraries produced:
#   libvterm.so     — the pure-C terminal state machine (~5000
#                     lines, no external deps — no ncurses, no
#                     libtool, no glib). System.loadLibrary("vterm").
...
#
# Layout mirrors moonlight-core: top-level dispatcher (jni/Android.mk)
# sets LOCAL_PATH=jni/, this file lives in jni/libs/vterm_jni/, and
# we restore LOCAL_PATH to jni/libs/vterm_jni/ via MY_LOCAL_PATH so
# the relative LOCAL_SRC_FILES and LOCAL_C_INCLUDES below resolve
# against THIS directory — not the dispatcher. ndk-build prepends
# NDK_PROJECT_PATH (.) on top, so the final src path is
# `./libs/deps/libvterm/src/...` relative to
# NDK_PROJECT_PATH=remoteClientLib/.
